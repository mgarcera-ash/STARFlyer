...
export default function StationsAdminPage() {
  ...
  const handleFile = (file: File) => {
    ...
    Papa.parse<RawRow>(file, {
      ...
      error: () => {
        setStatus("error");
        setErrorMsg("Failed to parse CSV. Make sure it's a valid Police Stations export.");
      },
    });
  };
  ...
  const handleLoad = async () => {
    setStatus("loading");
    try {
      const res = await fetch("/api/stations/load", {
        method: "POST",
              ...
              {status === "parsing" ? (
                ...
                <>
                  ...
                  <p style={{ fontFamily: "var(--font-sans)", fontSize: 12, color: "var(--muted)" }}>
                    Chicago Police Stations export
                  </p>
                </>
              )}
            </div>
        ...
        >
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: 14, color: "rgba(255,255,255,0.6)", fontFamily: "var(--font-sans)", lineHeight: 1.65 }}>
              The browser reads the CSV file locally using the File API — nothing is uploaded until you confirm. papaparse parses it into rows, which are sent to the database in one request. Existing stations are updated by district number; new ones are added. Safe to run whenever the City publishes a new export.
            </p>
          </div>
        </div>
      )}
